...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void MyInput()
    {
        ...
        crouching2 = Input.GetKey(KeyCode.C);

        isSprinting = Input.GetKey(KeyCode.LeftShift) && !isExhausted && y > 0 && grounded;
    ...
    {
        bool isMoving = (x != 0 || y != 0) && grounded && !isExhausted;

        if (isMoving)
        {
            float drainRate = staminaDrainRate * Time.deltaTime;
            if (!isSprinting)
            {
                drainRate *= 0.5f;
            }
            UseStamina(drainRate);
            timeSinceLastStaminaUse = 0f;
        }
        else
        {
            timeSinceLastStaminaUse += Time.deltaTime;
            if (timeSinceLastStaminaUse >= staminaRegenDelay && currentStamina < maxStamina && !isExhausted)
            {
        ...
        else if (currentStamina >= maxStamina * 0.3f && isExhausted)
        {
            SetExhausted(false);
        }
    }
    ...
    private void UpdateStaminaUI()
    {
        ...
        if (staminaFillImage != null)
        {
            ...
            staminaFillImage.color = Color.Lerp(lowStaminaColor, fullStaminaColor, staminaPercent);
            if (staminaPercent < 0.3f)
            {
                Color pulseColor = Color.Lerp(lowStaminaColor, fullStaminaColor, Mathf.PingPong(Time.time * 5f, 1f));
                staminaFillImage.color = pulseColor;
            }
        }
    }
    ...
    private void Movement()
    {
        ...
        if (isExhausted)
        {
            num3 *= exhaustedSpeedMultiplier;
        }

        if (crouching && grounded && readyToJump || crouching2 && grounded && readyToJump)
        {
        ...
        if (grounded && crouching || grounded && crouching2)
        {
            num5 = 0f;
        }
        if (wallRunning)
        ...
        float speedMultiplier = isExhausted ? exhaustedSpeedMultiplier : 1f;

        rb.AddForce(orientation.transform.forward * y * moveSpeed * Time.deltaTime * num4 * num5 * speedMultiplier);
        rb.AddForce(orientation.transform.right * x * moveSpeed * Time.deltaTime * num4 * speedMultiplier);
    }
    ...
    private void CounterMovement(float x, float y, Vector2 mag)
    {
        ...
        float currentMaxSpeed = isExhausted ? walkSpeed * exhaustedSpeedMultiplier : walkSpeed;

        if (Mathf.Sqrt(Mathf.Pow(rb.velocity.x, 2f) + Mathf.Pow(rb.velocity.z, 2f)) > currentMaxSpeed)
        {
            float num3 = rb.velocity.y;
            Vector3 vector = rb.velocity.normalized * currentMaxSpeed;
    ...
    // Public methods for stamina (можно использовать для внешнего доступа)
